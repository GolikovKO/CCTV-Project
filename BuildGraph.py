...
from db_functions import getTimeAndPeople, getPodezd


def build_graph():
    people_data = getTimeAndPeople()
    ts_data = getPodezd()
    ...
    for i in toDateTime_ts:
        ax.text(i, max(x_people) + 0.2, str(ts_data[inside_list][4]) + str(ts_data[inside_list][6]) + '(' + str(
            ts_data[inside_list][3]) + ')', fontsize=12, horizontalalignment='center')
        inside_list += 1
